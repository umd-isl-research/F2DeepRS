...
from F2deepRS_net import F2deepRS
import os
from tqdm import tqdm
import config
from utils import save_model, load_model
from torch.optim import Adam
from dataset import YahooR2Dataset, get_YahooR2_dataloader, load_json_data, get_batches#, YahooR2Dataset_train, YahooR2Dataset_test
from letor_metrics import ndcg_from_ranking, ndcg_score
...
def eval_model(net, data_test):
    ...
    e_ndcg = 0
    num_recommend_items = []
    for u, gt in tqdm(data_test.testing.items()):
        ...
        u,i,l = torch.from_numpy(u).long().to(device), torch.from_numpy(i).long().to(device), torch.from_numpy(l).float().unsqueeze(1).to(device)
...
if __name__=="__main__":
    ...
    f2deepRS = F2deepRS(config.num_user, config.num_item, config.vec_dim)
    f2deepRS = load_model(f2deepRS, os.path.join(config.model_root, "None"))  # load existing model, or randomize the weights
    f2deepRS.init_embeddings()  # load initial user and item vectors learned by BPR
    f2deepRS.to(device)
    f2deepRS.eval()
    tr_acc, tr_loss, tr_f1 = [], [], []
    te_acc, te_loss, te_recall, te_neg_recall, te_precision, te_ndcg = [], [], [], [], [], []
    eval_acc, eval_loss, eval_f1, eval_recall, eval_neg_recall, eval_precision, eval_ndcg, recN = eval_model(f2deepRS, data_test)
    epoch_recN = []
    ...
    opt = Adam([
        {"params":f2deepRS.user_embeddings.parameters(), "lr":config.lr_embedding},
        {"params":f2deepRS.item_embeddings.parameters(), "lr":config.lr_embedding},
        {"params":f2deepRS.layers.parameters()}],
    ...
    for e in range(config.epoch):
        ...
        for u,i,l in get_batches(data_train):
            u,i,l = torch.tensor(u).long().to(device), torch.tensor(i).long().to(device), torch.tensor(l).unsqueeze(1).float().to(device)
            opt.zero_grad()
            logits = f2deepRS(u,i)
        ...
        if (e+1)%config.verbose==0:
            f2deepRS.eval()
            eval_acc, eval_loss, eval_f1, eval_recall, eval_neg_recall, eval_precision, eval_ndcg, recN = eval_model(f2deepRS, data_test=data_test)
            epoch_recN.append(recN)
            f2deepRS.train()
            te_acc.append(eval_acc)
            te_loss.append(eval_loss)
            ...
            if best_eval_acc<eval_acc:
                ...
                best_model = copy.deepcopy(f2deepRS)  # just want to take a look at the best one
            print("Evaluation: acc={:.6f}, loss={:.6f}, f1={:.6f}".format(eval_acc, eval_loss, eval_f1),
                  "recall={:.6f}, neg_recall={:.6f}, precision={:.6f}, ndcg={:.6f}".format(eval_recall, eval_neg_recall, eval_precision, eval_ndcg))


    save_model(f2deepRS, "deepRS%d.pt" % time.time())
